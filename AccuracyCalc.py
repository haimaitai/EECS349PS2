...
class AccCalculator:
        def __init__(self, file_name, prune=False):
                self.samples = importData.Samples(file_name)
                self.builder = treeBuild.treeBuilder()
                self.file_name = file_name.split('/')[-1]
                self.prune = prune
                if prune:
                        print "(In theory) Implementing tree with pruning..."
                ...
                for i in step:
                        for sample in samples:
                                ...
                                if not node_counts.get(i):
                                        node_counts[i] = 0
                                (tr, te) = sample
                                tree = self.builder.create(tr[0:i], self.samples.attributes(), self.prune)
                                counts[i] += self.test(tree, te)
                                node_counts[i] += tree.numNodes()
                                total = len(te)
